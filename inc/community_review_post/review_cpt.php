...
// Community review Show Reports

function wpt_show_commreply_report() {
	add_meta_box(
		'wp_comm_report_id',
		'Reply Reports',
		'wp_comm_reportreply_callback',
		'wp_cfpost_review',
		'advanced',
		'default'
	);
}
add_action( 'add_meta_boxes', 'wpt_show_commreply_report' );

function wp_comm_reportreply_callback ( $post_id) {

    $comm_post_report = get_post_meta($post_id->ID, 'comm_repost_on_post_val', true);
    ?>
     <table class="table">
    <thead>
      <tr>
